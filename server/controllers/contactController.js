...
export const handleContactSubmit = async (req, res) => {
  ...
  // Check if SMTP environment parameters are available
  const canSendEmail =
  ...
  if (canSendEmail) {
    try {
      const transporter = nodemailer.createTransport({
        ...
        secure: process.env.EMAIL_SMTP_PORT === "465" || !process.env.EMAIL_SMTP_PORT, // true for 465, false for other ports
        auth: {
      ...
      console.log(`[MAIL] Message from ${name} sent successfully via SMTP.`);
      ...
      console.error("[MAIL ERROR] Failed to send email via Nodemailer:", err.message);
      // Fall through to console fallback if SMTP fails
    }
  }
